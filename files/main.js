...
function add(resource, sibling) {
    ...
    creator
        ...
        .append(new ElementCreator("p").text("Student: " + (resource.isStudent ? "Yes" : "No")));
    ...
    if (sibling) {
        parent.replaceChild(creator.element, sibling);
    } else {
...
function edit(resource) {
    ...
    const formCreator = new ElementCreator("form")
        .id(resource.idforDOM + "-edit-form")
    ...
    */
    ...
    formCreator
        ...
        .append(new ElementCreator("label").text("Is Student").with("for", "resource-isStudent"))
        .append(new ElementCreator("input").id("resource-isStudent").with("type", "checkbox").with("checked", resource.isStudent));
        ...
        .append(new ElementCreator("button").text("Save").listener('click', (event) => {
            ...
            */
            ...
            const updatedResource = {
                ...
                age: Number(document.getElementById("resource-age").value),
                isStudent: document.getElementById("resource-isStudent").checked
            };

        if (updatedResource.age < 0 || updatedResource.age > 120) {
            alert("Age must be between 0 and 120.");
            return;
        }

        if (updatedResource.isStudent && updatedResource.age < 18) {
            alert("To be a student, you must be 18 years or older.");
            return;
        }
            ...
            */
            ...
            .then(updatedFromServer => {
                ...
                Object.assign(resource, updatedFromServer);
                formCreator.element.remove();

                const oldArticle = document.getElementById(resource.idforDOM);
                add(resource, oldArticle);
            })
            .catch(err => console.error(err));
        }))

        .insertBefore(document.querySelector('main'), document.getElementById(resource.idforDOM));
}
...
function create() {
    const formCreator = new ElementCreator("form")
        .id("create-form")
        .append(new ElementCreator("h3").text("Create New Person"));

    formCreator
        .append(new ElementCreator("label").text("Name").with("for", "create-name"))
        .append(new ElementCreator("input").id("create-name").with("type", "text"))

        .append(new ElementCreator("label").text("Age").with("for", "create-age"))
        .append(new ElementCreator("input").id("create-age").with("type", "number"))

        .append(new ElementCreator("label").text("Is Student").with("for", "create-isStudent"))
        .append(new ElementCreator("input").id("create-isStudent").with("type", "checkbox"));
    ...
    formCreator
        .append(new ElementCreator("button").text("Save").listener('click', (event) => {
            ...
            const newResource = {
                ...
                isStudent: document.getElementById("create-isStudent").checked
            };

            if (newResource.age < 0 || newResource.age > 120) {
                alert("Age must be between 0 and 120.");
                return;
            }

            if (newResource.isStudent && newResource.age < 18) {
                alert("To be a student, you must be 18 years or older.");
                return;
            }
            ...
            .then(createdResource => {
                add(Object.assign(new Person(), createdResource));
                document.getElementById("create-form").remove();
            })
            .catch(err => console.error(err));
        }))
        .insertBefore(document.querySelector('main'), document.querySelector('#bottom'));
}
    ...
    fetch("/api/resources")
        ...
        .then(resources => {
            for (const resource of resources) {
                add(Object.assign(new Person(), resource));
            }
        });
